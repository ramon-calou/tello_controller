...
#############################################
#                                           #
# Library to robot control using different  #
# approaches.                               #
#                                           #
...
def cartesian_control(robot_pose,goal,K_v,K_omega,max_lin=0.5, max_ang=0.5):
    """
    ...
    error_lin = round(math.sqrt(error_x**2 + error_y**2),2)
    v = K_v*error_lin
...
def polar_control(robot_pose,goal,K_rho,K_alpha, K_beta, max_lin=0.5, max_ang=0.5):
    """
    This function computes the control signal to guides the 
    robot to the desired goal. It's based on the Cartesian
    Control Algorithm
